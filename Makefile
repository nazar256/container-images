...
smoke-image: podman-check
	...
	@if [ "$(IMAGE)" = "telegram-transcribe-bot" ]; then \
		...
		podman exec "$${ctr_id}" sh -ceu 'for i in $$(seq 1 3); do status="$$(curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/mcp || true)"; case "$${status}" in 400|405|406) ;; *) echo "expected unauthenticated /mcp invalid request to return protocol error, got HTTP $${status}" >&2; exit 1 ;; esac; done; sse_status="$$(curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/sse || true)"; if [ "$${sse_status}" != 404 ]; then echo "expected /sse disabled with HTTP 404, got HTTP $${sse_status}" >&2; exit 1; fi'; \
		...
		podman exec "$${api_key_ctr_id}" sh -ceu 'unauth_status="$$(curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/mcp || true)"; if [ "$${unauth_status}" != 401 ]; then echo "expected /mcp without X-API-Key to return HTTP 401, got HTTP $${unauth_status}" >&2; exit 1; fi; auth_status="$$(curl -sS -H "X-API-Key: dummy-incoming-api-key" -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/mcp || true)"; case "$${auth_status}" in 400|405|406) ;; *) echo "expected /mcp with X-API-Key to pass auth and return protocol error, got HTTP $${auth_status}" >&2; exit 1 ;; esac'; \
		podman exec "$${api_key_ctr_id}" sh -ceu 'processes="$$(ps -ww -eo args=)"; mcp_proxy_count="$$(printf "%s\n" "$${processes}" | grep -Ec "(^|/)[m]cp-proxy( |$$)" || true)"; chrome_devtools_count="$$(printf "%s\n" "$${processes}" | grep -Ec "(^|/)[c]hrome-devtools-mcp( |$$)" || true)"; if [ "$${mcp_proxy_count}" -ne 1 ] || [ "$${chrome_devtools_count}" -ne 1 ]; then echo "unexpected startup process counts: mcp-proxy=$${mcp_proxy_count} chrome-devtools-mcp=$${chrome_devtools_count}" >&2; printf "%s\n" "$${processes}" >&2; exit 1; fi'; \
		...
		podman run --rm --entrypoint bash $(LOCAL_TAG_PREFIX)$(IMAGE):test -ceu 'command -v mcpproxy >/dev/null && command -v node >/dev/null && command -v npm >/dev/null && command -v npx >/dev/null && command -v python3 >/dev/null && command -v uv >/dev/null && command -v uvx >/dev/null && command -v dbus-run-session >/dev/null && command -v gnome-keyring-daemon >/dev/null && command -v secret-tool >/dev/null && test -x /usr/local/bin/docker-entrypoint.sh && test -x /usr/local/bin/keyring-session.sh && test -x /usr/local/lib/mcpproxy/wrappers/run-nextcloud-mcp'; \
		...
		podman run --rm $(LOCAL_TAG_PREFIX)$(IMAGE):test sh -lc 'test "$$(id -u)" = 10001; test -z "$${DISPLAY:-}"; test -z "$${WAYLAND_DISPLAY:-}"; ! env | grep -Eqi "(keyring.*password|password.*keyring)"; test -n "$${DBUS_SESSION_BUS_ADDRESS:-}"; printf %s startup-probe | secret-tool store --label=startup-probe service __keyring_startup_probe__; test "$$(secret-tool lookup service __keyring_startup_probe__)" = startup-probe; secret-tool clear service __keyring_startup_probe__'; \
		podman run --rm $(LOCAL_TAG_PREFIX)$(IMAGE):test sh -lc 'exec mcpproxy --version' >/dev/null; \
		...
		ready=false; for i in $$(seq 1 300); do if podman exec "$${ctr_id}" curl -fsS http://127.0.0.1:8080/api/v1/status -H "X-API-Key: dummy-admin-api-key" >/dev/null 2>&1; then ready=true; break; fi; if [ "$$(podman inspect -f '{{.State.Running}}' "$${ctr_id}")" != true ]; then echo 'mcpproxy container exited before becoming ready' >&2; podman logs "$${ctr_id}" >&2; podman inspect -f 'exit={{.State.ExitCode}} error={{.State.Error}}' "$${ctr_id}" >&2; exit 1; fi; sleep 0.2; done; if [ "$${ready}" != true ]; then echo 'mcpproxy did not become ready within 60 seconds' >&2; podman logs "$${ctr_id}" >&2; podman exec "$${ctr_id}" ps -ww -eo pid,ppid,args >&2 || true; exit 1; fi; \
		podman exec "$${ctr_id}" bash -ceu 'unauth_status="$$(curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:8080/mcp/call || true)"; if [ "$${unauth_status}" != "401" ]; then echo "expected /mcp/call without auth to return HTTP 401, got HTTP $${unauth_status}" >&2; exit 1; fi; processes="$$(ps -ww -eo args=)"; mcpproxy_count="$$(printf "%s\n" "$${processes}" | grep -Ec "(^|/)[m]cpproxy( |$$)" || true)"; dbus_count="$$(printf "%s\n" "$${processes}" | grep -Ec "(^|/)[d]bus-daemon( |$$)" || true)"; keyring_count="$$(printf "%s\n" "$${processes}" | grep -Ec "(^|/)[g]nome-keyring-daemon( |$$)" || true)"; if [ "$${mcpproxy_count}" -ne 1 ] || [ "$${dbus_count}" -ne 1 ] || [ "$${keyring_count}" -ne 1 ]; then echo "unexpected process counts: mcpproxy=$${mcpproxy_count} dbus=$${dbus_count} keyring=$${keyring_count}" >&2; printf "%s\n" "$${processes}" >&2; exit 1; fi'; \
		podman rm -f "$${ctr_id}" >/dev/null; \
