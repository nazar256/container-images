...
smoke-image: podman-check
	...
	@if [ "$(IMAGE)" = "telegram-transcribe-bot" ]; then \
		...
		podman exec "$${unauth_ctr_id}" sh -ceu 'ps -ww -eo args= | grep -Eq "(^|/)[c]hrome-devtools-mcp( |$$).+ --headless( |$$)"'; \
